...
export function ChatWidget() {
  ...
  const processUserMessage = (userMessage: string) => {
    ...
    if (lowerCaseMessage.includes('services')) {
      respondWithServices();
    } else if (lowerCaseMessage.includes('child defense institute') || lowerCaseMessage.includes('cdi') || lowerCaseMessage.includes('child defense') || lowerCaseMessage.includes('what is child defense institute')) {
      respondWithCDIInfo();
    } else {
    ...
    bgChatFlow.cdi.forEach((info, index) => {
      setTimeout(() => {
        if (index === bgChatFlow.cdi.length - 1) {
          ...
          // Split the string by newline characters and create bullet points
          const bulletPoints = info.split('\n').map((point, i) => (
            <li key={i}>{point}</li>
          ));
          addMessage(<ul className="list-disc list-inside">{bulletPoints}</ul>, false);
        } else {
